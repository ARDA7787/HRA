...
class InteractiveVisualizer:
    ...
    def plot_time_series_with_anomalies(
        ...
        fig = make_subplots(
            ...
            shared_xaxes=True,
            subplot_titles=(
                ["EDA (Electrodermal Activity)", "HR (Heart Rate)", "Uncertainty"]
                if uncertainty is not None
                else ["EDA (Electrodermal Activity)", "HR (Heart Rate)"]
            ),
        ...
        fig.update_xaxes(
            title_text="Time (seconds)", row=3 if uncertainty is not None else 2, col=1
        )
    ...
    def plot_roc_pr_curves(
        ...
        # Update layout
        fig.update_layout(title=title, height=500, template=self.theme)
    ...
    def create_dashboard(
        self, results_dict: Dict[str, Any], output_path: Optional[Path] = None
    ) -> str:
        """
        ...
        for plot_name, plot_data in results_dict.items():
            if isinstance(plot_data, go.Figure):
                ...
                html_parts.append(
                    f'<div class="section"><div id="{plot_id}" class="plot-container"></div></div>'
                )
...
def create_comprehensive_report(
    ...
    fig_cm = visualizer.plot_confusion_matrix(
        y_true, y_pred, title="Confusion Matrix", output_path=output_dir / "confusion_matrix.html"
    )
    ...
    # Create dashboard
    dashboard_html = visualizer.create_dashboard(plots, output_path=output_dir / "dashboard.html")
