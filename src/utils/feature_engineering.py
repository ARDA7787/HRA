...
class PhysiologicalFeatureExtractor:
    ...
    def extract_statistical_features(
        self, signal: np.ndarray, prefix: str = ""
    ) -> Dict[str, float]:
        ...
        try:
            features = {
                ...
                f"{prefix}iqr": float(
                    np.percentile(clean_signal, 75) - np.percentile(clean_signal, 25)
                ),
                f"{prefix}skewness": (
                    float(stats.skew(clean_signal)) if len(clean_signal) > 2 else 0.0
                ),
                f"{prefix}kurtosis": (
                    float(stats.kurtosis(clean_signal)) if len(clean_signal) > 3 else 0.0
                ),
    ...
    def extract_frequency_features(self, signal: np.ndarray, prefix: str = "") -> Dict[str, float]:
        ...
        if len(signal) < 4 or np.all(np.isnan(signal)):
            return {
                ...
                f"{prefix}spectral_spread": 0.0,
                f"{prefix}power_low": 0.0,
                f"{prefix}power_high": 0.0,
        ...
        try:
            ...
            power_high = (
                np.sum(psd[high_band_mask]) / total_power if np.any(high_band_mask) else 0.0
            )
    ...
    def extract_time_domain_features(
        ...
        if len(signal) < 4 or np.all(np.isnan(signal)):
            return {
                f"{prefix}slope": 0.0,
                f"{prefix}num_peaks": 0.0,
                f"{prefix}peak_density": 0.0,
                f"{prefix}zero_crossings": 0.0,
                f"{prefix}signal_energy": 0.0,
                f"{prefix}autocorr_lag1": 0.0,
                f"{prefix}variability": 0.0,
            }
    ...
    def extract_cross_signal_features(self, eda: np.ndarray, hr: np.ndarray) -> Dict[str, float]:
        ...
        if len(eda) != len(hr) or len(eda) < 4:
            return {
                "eda_hr_correlation": 0.0,
                "eda_hr_lag_correlation": 0.0,
                "eda_hr_coherence": 0.0,
                "eda_hr_phase_sync": 0.0,
            }
    ...
    def extract_all_features(
        self, eda_window: np.ndarray, hr_window: np.ndarray
    ) -> Dict[str, float]:
        """
...
def build_feature_matrix(
    X: np.ndarray, window_size: int, step: int, fs: float = 4.0
...
def get_feature_importance_ranking(
    feature_matrix: np.ndarray, labels: np.ndarray, feature_names: List[str]
    ...
    try:
        ...
        importance_ranking = sorted(zip(feature_names, mi_scores), key=lambda x: x[1], reverse=True)
