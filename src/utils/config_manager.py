...
class DataConfig:
    ...
    eda: Dict[str, Any] = field(
        default_factory=lambda: {"lowpass_filter": True, "cutoff_frequency": 1.0, "filter_order": 4}
    )
...
class EvaluationConfig:
    ...
    cross_validation: Dict[str, Any] = field(
        default_factory=lambda: {"enabled": True, "method": "stratified", "n_splits": 5, "gap": 0}
    )
    subject_independent: Dict[str, bool] = field(default_factory=lambda: {"enabled": True})
    uncertainty_quantification: Dict[str, Any] = field(
        default_factory=lambda: {"enabled": True, "n_bootstrap": 100, "bootstrap_ratio": 0.8}
    )
    metrics: list = field(
        default_factory=lambda: [
            "roc_auc",
        ...
        if "models" in config_dict:
            for model_name, model_dict in config_dict["models"].items():
                config.models[model_name] = ModelConfig(
                    enabled=model_dict.get("enabled", True), params=model_dict.get("params", {})
                )
...
def get_config_from_env() -> Dict[str, Any]:
    ...
    if "ANOMALY_DEVICE" in os.environ:
        ...
        for model_name in ["autoencoder", "vae", "lstm", "transformer"]:
            env_config["models"].setdefault(model_name, {}).setdefault("params", {})[
                "device"
            ] = device
