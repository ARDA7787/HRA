...
class ModelSerializer:
    ...
    def save_model(self, model: Any, model_name: str, 
        ...
        metadata.update({
            ...
            'torch_version': torch.__version__ if hasattr(model, 'model') and isinstance(getattr(model, 'model'), torch.nn.Module) else None
        })
        ...
        try:
            ...
            if hasattr(model, 'model') and isinstance(getattr(model, 'model'), torch.nn.Module):
                ...
                torch.save({
                    ...
                    'optimizer_state_dict': getattr(model, 'optimizer', {}).state_dict() if hasattr(getattr(model, 'optimizer', {}), 'state_dict') else None
                }, model_dir / "model.pth")
        ...
        try:
            ...
            if (model_dir / "model.pth").exists():
                ...
                if model_class_name == 'AEAnomalyDetector':
                    ...
                    model.model = DenseAE(init_params.get('input_dim', 2), init_params.get('latent_dim', 32))
                    model.model.load_state_dict(checkpoint['model_state_dict'])
                    
                elif model_class_name == 'VAEAnomalyDetector':
                    from ..models.vae_anomaly import VAEAnomalyDetector, VAE
                    init_params = checkpoint.get('model_init_params', {})
                    model.model = VAE(init_params.get('input_dim', 2), init_params.get('latent_dim', 32))
                    model.model.load_state_dict(checkpoint['model_state_dict'])
                    ...
                    from ..models.transformer_anomaly import TransformerAnomalyDetector, TransformerEncoder
                    init_params = checkpoint.get('model_init_params', {})
    ...
    def get_model(self, model_name: str, version: str = 'latest') -> tuple[Any, Dict[str, Any]]:
        ...
        if version == 'latest':
            ...
            latest_version = max(model_versions.keys(), 
                               key=lambda v: model_versions[v]['registered_timestamp'])
            ...
            latest_prod = max(prod_versions.keys(),
                            key=lambda v: prod_versions[v]['registered_timestamp'])
