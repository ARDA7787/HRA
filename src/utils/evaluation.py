        ...
        if groups is not None:
            ...
            for i in range(self.n_splits):
                ...
                train_idx = np.concatenate(
                    [np.arange(0, train_end), np.arange(train_start, n_samples)]
                )
...
class AnomalyEvaluator:
    ...
    def uncertainty_quantification(
        self, model: Any, X: np.ndarray, n_bootstrap: int = 100, bootstrap_ratio: float = 0.8
    ) -> Dict[str, np.ndarray]:
        """
...
def plot_evaluation_curves(
    ...
    ax2.plot(
        recall, precision, color="darkorange", lw=2, label=f"PR curve (AP = {avg_precision:.3f})"
    )
    ax2.axhline(y=np.sum(y_true) / len(y_true), color="navy", linestyle="--", label="Random")
