...
def prepare_data(df: pd.DataFrame, config: Any) -> tuple:
    ...
    if config.data.eda.get('lowpass_filter', True):
        eda_f = pd.Series(butter_lowpass(
            eda.values, 
            fs=config.data.fs, 
            cutoff=config.data.eda.get('cutoff_frequency', 1.0)
        ))
    else:
        eda_f = eda.copy()
...
def train_single_model(model_type: str, X_train: np.ndarray, 
                      config: Any, model_configs: Dict[str, Any]) -> Any:
...
def evaluate_models(models: Dict[str, Any], X_test: np.ndarray, y_test: np.ndarray,
                   config: Any) -> Dict[str, Dict[str, float]]:
    """Evaluate all models."""
    results = {}
    ...
    for model_name, model in models.items():
        ...
        try:
            ...
            logging.info(f"{model_name} - ROC AUC: {metrics.get('roc_auc', 'N/A'):.4f}, "
...
def advanced_evaluation(models: Dict[str, Any], X: np.ndarray, y: np.ndarray,
                       config: Any, subjects: Optional[np.ndarray] = None) -> Dict[str, Any]:
...
def create_visualizations(time: np.ndarray, eda: np.ndarray, hr: np.ndarray,
...
def save_models(models: Dict[str, Any], model_results: Dict[str, Dict[str, float]], 
               config: Any, output_dir: Path) -> None:
...
def main():
    ...
    parser.add_argument('--csv', type=str, required=True, 
                       help='Input CSV with columns: time, EDA, HR, [label]')
    parser.add_argument('--config', type=str, 
                       help='Configuration file path')
    parser.add_argument('--output_dir', type=str, default='outputs',
                       help='Output directory')
    parser.add_argument('--subject_id', type=str,
                       help='Subject ID for subject-independent evaluation')
    parser.add_argument('--models', type=str, nargs='+', 
                       default=['isolation_forest', 'autoencoder', 'vae'],
                       help='Models to train')
    parser.add_argument('--ensemble', action='store_true',
                       help='Train ensemble model')
    parser.add_argument('--advanced_eval', action='store_true',
                       help='Perform advanced evaluation')
    ...
    if y_window is not None:
        logging.info(f"Labels available: {len(y_window)} windows, "
