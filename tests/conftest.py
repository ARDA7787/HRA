...
def sample_physiological_data():
    ...
    return pd.DataFrame({"time": time, "EDA": eda, "HR": hr, "label": labels})
...
def mock_config():
    ...
    config.models = {
        "isolation_forest": ModelConfig(
            enabled=True, params={"contamination": 0.1, "random_state": 42}
        ),
        "autoencoder": ModelConfig(
            enabled=True, params={"latent_dim": 32, "epochs": 2, "batch_size": 32}
        ),
    }
...
def sample_model_results():
    ...
    return {
        "isolation_forest": {"roc_auc": 0.87, "f1": 0.82, "precision": 0.85, "recall": 0.79},
        "autoencoder": {"roc_auc": 0.91, "f1": 0.86, "precision": 0.88, "recall": 0.84},
        "ensemble": {"roc_auc": 0.94, "f1": 0.90, "precision": 0.92, "recall": 0.88},
    }
